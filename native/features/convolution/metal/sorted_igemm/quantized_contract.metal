...
template <int bits, int in_channels, int out_channels>
inline void sparse_quantized_igemm_kernel(
    device const half* feats,
    device const uint* weights,
    device const half* scales,
    device const half* biases,
    ...
    sparse_quantized_igemm_impl<bits, in_channels, out_channels>(
        feats,
        weights,
        scales,
        biases,
...
void sparse_quantized_igemm_f16_b4_cin32_cout32(
    ...
    threadgroup half lhs[64 * (32 + 8)];
    threadgroup half rhs[32 * (32 + 8)];
    sparse_quantized_igemm_kernel<4, 32, 32>(
        feats,
        weights,
        scales,
        biases,
...
[[kernel, max_total_threads_per_threadgroup(128)]]
void sparse_quantized_igemm_f16_b8_cin32_cout32(
    device const half* feats [[buffer(0)]],
    device const uint* weights [[buffer(1)]],
    device const half* scales [[buffer(2)]],
    device const half* biases [[buffer(3)]],
    ...
    threadgroup half lhs[64 * (32 + 8)];
    threadgroup half rhs[32 * (32 + 8)];
    sparse_quantized_igemm_kernel<8, 32, 32>(
        feats,
        weights,
        scales,
        biases,
...
[[kernel, max_total_threads_per_threadgroup(128)]]
void sparse_quantized_igemm_f16_b4_cin32_cout64(
    device const half* feats [[buffer(0)]],
    device const uint* weights [[buffer(1)]],
    device const half* scales [[buffer(2)]],
    device const half* biases [[buffer(3)]],
    ...
    threadgroup half lhs[64 * (32 + 8)];
    threadgroup half rhs[32 * (32 + 8)];
    sparse_quantized_igemm_kernel<4, 32, 64>(
        feats,
        weights,
        scales,
        biases,
...
[[kernel, max_total_threads_per_threadgroup(128)]]
void sparse_quantized_igemm_f16_b8_cin32_cout64(
    device const half* feats [[buffer(0)]],
    device const uint* weights [[buffer(1)]],
    device const half* scales [[buffer(2)]],
    device const half* biases [[buffer(3)]],
    ...
    threadgroup half lhs[64 * (32 + 8)];
    threadgroup half rhs[32 * (32 + 8)];
    sparse_quantized_igemm_kernel<8, 32, 64>(
        feats,
        weights,
        scales,
        biases,
...
[[kernel, max_total_threads_per_threadgroup(128)]]
void sparse_quantized_igemm_f16_b4_cin64_cout32(
    device const half* feats [[buffer(0)]],
    device const uint* weights [[buffer(1)]],
    device const half* scales [[buffer(2)]],
    device const half* biases [[buffer(3)]],
    ...
    threadgroup half lhs[64 * (64 + 8)];
    threadgroup half rhs[64 * (32 + 8)];
    sparse_quantized_igemm_kernel<4, 64, 32>(
        feats,
        weights,
        scales,
        biases,
...
[[kernel, max_total_threads_per_threadgroup(128)]]
void sparse_quantized_igemm_f16_b8_cin64_cout32(
    device const half* feats [[buffer(0)]],
    device const uint* weights [[buffer(1)]],
    device const half* scales [[buffer(2)]],
    device const half* biases [[buffer(3)]],
    ...
    threadgroup half lhs[64 * (64 + 8)];
    threadgroup half rhs[64 * (32 + 8)];
    sparse_quantized_igemm_kernel<8, 64, 32>(
        feats,
        weights,
        scales,
        biases,
...
[[kernel, max_total_threads_per_threadgroup(128)]]
void sparse_quantized_igemm_f16_b4_cin64_cout64(
    device const half* feats [[buffer(0)]],
    device const uint* weights [[buffer(1)]],
    device const half* scales [[buffer(2)]],
    device const half* biases [[buffer(3)]],
    ...
    threadgroup half lhs[64 * (64 + 8)];
    threadgroup half rhs[64 * (32 + 8)];
    sparse_quantized_igemm_kernel<4, 64, 64>(
        feats,
        weights,
        scales,
        biases,
...
[[kernel, max_total_threads_per_threadgroup(128)]]
void sparse_quantized_igemm_f16_b8_cin64_cout64(
    device const half* feats [[buffer(0)]],
    device const uint* weights [[buffer(1)]],
    device const half* scales [[buffer(2)]],
    device const half* biases [[buffer(3)]],
    ...
    threadgroup half lhs[64 * (64 + 8)];
    threadgroup half rhs[64 * (32 + 8)];
    sparse_quantized_igemm_kernel<8, 64, 64>(
        feats,
        weights,
        scales,
        biases,
