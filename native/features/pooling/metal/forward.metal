...
[[kernel]] void sparse_pool_relation_f16_i32(
    ...
    int channel = int(elem) - out_row * channels;
    if (out_row >= counts[1]) {
        out[elem] = reduce == 1 ? half(-INFINITY) : half(0.0f);
        return;
    }
    ...
    for (int edge = row_offsets[out_row]; edge < row_offsets[out_row + 1];
         ++edge) {
        int in_row = in_rows[edge];
        float value = float(feats[in_row * feat_s0 + channel * feat_s1]);
        acc = reduce == 1 ? max(acc, value) : acc + value;
        ++degree;
    }
    out[elem] = half(reduce == 2 ? acc / float(max(degree, 1)) : acc);
}
