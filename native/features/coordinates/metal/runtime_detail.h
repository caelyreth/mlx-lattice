...
mx::array make_int32_temp(int elements) {
    return backend::metal::make_temp<std::int32_t>(
        static_cast<std::size_t>(std::max(elements, 1))
    );
}
...
void bind_input_arrays(
    ...
    backend::metal::bind_input_arrays(encoder, inputs, first);
}
...
template <typename Library, typename Encoder>
void encode_stable_compact_offsets(
    ...
    auto scan = backend::metal::lattice_kernel(
        stream, "scan_coord_set_selected_blocks_i32", library
    );
...
void encode_relation_compact_offsets(
    ...
    auto prefix = backend::metal::lattice_kernel(
        stream, "prefix_relation_selected_blocks_i32", library
    );
    encoder.set_compute_pipeline_state(prefix);
    encoder.set_output_array(buffers.block_offsets, 0);
...
void encode_neighbor_row_offsets(
    ...
    if (query_rows < kParallelCompactThreshold) {
        auto prefix = backend::metal::lattice_kernel(
            stream, "prefix_neighbor_row_offsets_i32", library
        );
        encoder.set_compute_pipeline_state(prefix);
...
void clear_coord_hash(
    ...
    auto clear =
        backend::metal::lattice_kernel(stream, "coord_hash_clear_i32", library);
...
void insert_coord_hash(
    ...
    auto insert = backend::metal::lattice_kernel(
        stream, "coord_hash_insert_rows_i32", library
    );
...
void encode_relation_grouped_view(
    ...
    auto clear = backend::metal::lattice_kernel(
        stream, "clear_relation_grouped_view_i32", library
    );
    encoder.set_compute_pipeline_state(clear);
    ...
    auto count = backend::metal::lattice_kernel(
        stream, "count_relation_grouped_view_i32", library
    );
    ...
    if (shape.group_count >= kParallelCompactThreshold) {
        ...
        auto scan = backend::metal::lattice_kernel(
            stream, "scan_relation_grouped_view_blocks_i32", library
        );
        encoder.set_compute_pipeline_state(scan);
        ...
        auto finalize = backend::metal::lattice_kernel(
            stream, "finalize_relation_grouped_view_i32", library
        );
        encoder.set_compute_pipeline_state(finalize);
        encoder.set_input_array(buffers.local_offsets, 0);
        encoder.set_input_array(buffers.block_offsets, 1);
        ...
        auto prefix = backend::metal::lattice_kernel(
            stream, "prefix_relation_grouped_view_i32", library
        );
        encoder.set_compute_pipeline_state(prefix);
        encoder.set_output_array(outputs[RelationViewRowOffsets], 0);
    ...
    auto fill = backend::metal::lattice_kernel(
        stream, "fill_relation_grouped_view_i32", library
    );
...
void encode_relation_direct_view(
    ...
    auto clear = backend::metal::lattice_kernel(
        stream, "clear_relation_direct_view_i32", library
    );
    encoder.set_compute_pipeline_state(clear);
    ...
    auto fill = backend::metal::lattice_kernel(
        stream, "fill_relation_direct_view_i32", library
    );
    encoder.set_compute_pipeline_state(fill);
    encoder.set_input_array(inputs[0], 0);
    encoder.set_input_array(inputs[1], 1);
...
void compact_forward_relation_slots(
    ...
    if (shape.rows >= kParallelCompactThreshold) {
        ...
        auto count = backend::metal::lattice_kernel(
            stream, "count_forward_relation_slot_degrees_i32", library
        );
        encoder.set_compute_pipeline_state(count);
        ...
        auto count = backend::metal::lattice_kernel(
            stream, "count_forward_relation_slot_rows_i32", library
        );
        encoder.set_compute_pipeline_state(count);
        encoder.set_input_array(slots.in_rows, 0);
        encoder.set_input_array(slots.kernel_ids, 1);
        ...
        auto prefix = backend::metal::lattice_kernel(
            stream, "prefix_forward_relation_slot_rows_i32", library
        );
        encoder.set_compute_pipeline_state(prefix);
    ...
    auto compact = backend::metal::lattice_kernel(
        stream, "compact_forward_relation_slots_i32", library
    );
