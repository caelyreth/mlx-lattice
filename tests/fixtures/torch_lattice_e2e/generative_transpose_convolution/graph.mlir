module attributes {
  ...
  func.func @forward(
    ...
    %up = lattice.generative_conv_transpose3d %x, %up_weight, %up_bias {kernel_size = array<i64: 2, 1, 1>, stride = array<i64: 2, 1, 1>} : (!lattice.sparse_tensor<rank = 3, coord = batch_x_y_z, feature = row_channel, dtype = f32>, !lattice.weight<conv3d, f32>, !lattice.weight<bias, f32>) -> !lattice.sparse_tensor<rank = 3, coord = batch_x_y_z, feature = row_channel, dtype = f32>
