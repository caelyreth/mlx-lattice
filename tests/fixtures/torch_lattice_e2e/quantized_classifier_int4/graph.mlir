module attributes {
  ...
  func.func @forward(
    ...
    %stem_weight = lattice.weight @stem_weight {storage_key = "stem.weight", layout = #lattice.weight_layout<conv3d_o_zyx_i>, packing = #lattice.packing<int4, group_size = 32, scale_dtype = f16, mode = affine>} : !lattice.weight<conv3d, f32>
    ...
    %head_weight = lattice.weight @head_weight {storage_key = "head.weight", layout = #lattice.weight_layout<linear_o_i>, packing = #lattice.packing<int4, group_size = 32, scale_dtype = f16, mode = affine>} : !lattice.weight<linear, f32>
