module attributes {
  ...
  func.func @forward(
    ...
    %up = lattice.target_normalized_conv_transpose3d %source, %target, %up_weight, %up_bias {kernel_size = array<i64: 3, 1, 1>, stride = array<i64: 2, 1, 1>, padding = array<i64: 1, 0, 0>, dilation = array<i64: 1, 1, 1>, eps = 0.00000001 : f32, accumulation = "canonical_f32"} : (!lattice.sparse_tensor<rank = 3, coord = batch_x_y_z, feature = row_channel, dtype = f32>, !lattice.sparse_tensor<rank = 3, coord = batch_x_y_z, feature = row_channel, dtype = f32>, !lattice.weight<conv3d, f32>, !lattice.weight<bias, f32>) -> !lattice.sparse_tensor<rank = 3, coord = batch_x_y_z, feature = row_channel, dtype = f32>
