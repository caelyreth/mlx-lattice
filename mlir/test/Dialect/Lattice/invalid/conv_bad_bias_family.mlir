// Invalid: optional convolution bias must be a bias-family weight.
module attributes {
  ...
  func.func @forward(%coords: tensor<?x4xi32>,
    ...
    %not_bias = lattice.weight @stem.not_bias
      {storage_key = "stem.not_bias",
       layout = #lattice.weight_layout<conv3d_o_zyx_i>,
       packing = #lattice.packing<dense>}
      : !lattice.weight<conv3d, f16>
    %out = lattice.conv3d %input, %weight, %not_bias
      {kernel_size = array<i64: 1, 1, 1>, stride = array<i64: 1, 1, 1>,
       padding = array<i64: 0, 0, 0>, dilation = array<i64: 1, 1, 1>}
      : (!lattice.sparse_tensor<rank = 3, coord = batch_x_y_z,
                                feature = row_channel, dtype = f16>,
         !lattice.weight<conv3d, f16>,
         !lattice.weight<conv3d, f16>)
